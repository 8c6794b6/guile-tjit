...
(define (make-lattice g print? port)
  (define (step g)
    (let* ((copy (copy-graph g))
      ...
      copy))
  (define (loop g count)
    (if print? (display count port))
    (let ((lattice (step g)))
      (if print? (begin (display " -> " port)
                        (display (length (graph-nodes lattice)) port)))
      (let* ((new-g (reduce lattice))
             (new-count (length (graph-nodes new-g))))
        (if (= new-count count)
            (begin
              (if print? (newline port))
              new-g)
            (begin
              (if print? (begin (display " -> " port)
                                (display new-count port) (newline port)))
...
(define (test port)
  ...
  (map name
       (graph-nodes
        (make-lattice (make-graph a b c d any-node none-node) #t port))))
                                           ;;; note printflag #t
;(define (time-test)
;  (let ((t (runtime)))
;    (let ((ans (test)))
