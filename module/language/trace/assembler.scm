     ...
     (define-native (name (int dst) (int a) (int b))
       (let ((tmp r2)
         ...
         (jump (case (ref-type b)
               ...
               (bailout))
         (case (ref-type dst)
           ((gpr) (jit-movr (gpr dst) tmp))
           ((fpr) (gpr->fpr (fpr dst) tmp))
           ((mem) (memory-set! dst tmp))
           (else (err))))))))

(define-binary-arith-int-overflow %addov jit-boaddr jit-boaddi)
(define-binary-arith-int-overflow %subov jit-bosubr jit-bosubi)
...
(define-native (%div (int dst) (int a) (int b))
  (let ((tmp1-offset (moffs (make-memory -4)))
    ...
    (let* ((a/r (case (ref-type a)
      ...
      (jit-qdivr dst/r rem/r a/r b/r)
      (jump (jit-beqi rem/r (imm 0)) make-fixnum)
