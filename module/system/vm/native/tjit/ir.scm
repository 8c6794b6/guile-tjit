...
(define-syntax gen-put-index
  (syntax-rules (const
                 scm scm! fixnum fixnum! pair pair!
                 u64 u64! f64 f64!)
    ((_ ol) ol)
    ((_ ol (const arg) . rest)
     (gen-put-index ol . rest))
    ((_ ol (scm arg) . rest)
     (gen-read-index ol &scm arg rest))
    ((_ ol (scm! arg) . rest)
     (gen-write-index ol &scm arg rest))
    ((_ ol (fixnum arg) . rest)
     (gen-read-index ol &exact-integer arg rest))
    ((_ ol (fixnum! arg) . rest)
     (gen-write-index ol &exact-integer arg rest))
    ((_ ol (pair arg) . rest)
     (gen-read-index ol &pair arg rest))
    ((_ ol (pair! arg) . rest)
     (gen-write-index ol &pair arg rest))
    ((_ ol (u64 arg) . rest)
     (gen-read-index ol &u64 arg rest))
    ((_ ol (u64! arg) . rest)
     (gen-write-index ol &u64 arg rest))
    ((_ ol (f64 arg) . rest)
     (gen-read-index ol &f64 arg rest))
    ((_ ol (f64! arg) . rest)
     (gen-write-index ol &f64 arg rest))
    ((_ ol (other arg) . rest)
     (let* ((i (+ arg (outline-sp-offset ol)))
            (reads (outline-read-indices ol)))
       (unless (memq i reads)
...
(define-syntax gen-put-element-type
  (syntax-rules (const
    ...
    ((_ ol (scm! arg) . rest)
     (let* ((n (+ arg (outline-sp-offset ol)))
            (types (assq-set! (outline-types ol) n 'scm)))
           ...
           (test-proc
            (lambda (ol op locals)
              (let lp ((flags '(flag ...)) (ns (cdr op)))
                (match (cons flags ns)
                  (((f . flags) . (n . ns))
                   (debug 1 ";;; [define-ir] test-proc: ~s => ~s" n f)
                   (if (memq f '(fixnum flonum pair vector))
                       (let* ((v (pointer->scm (vector-ref locals n)))
                              (t (type-of v)))
                         (debug 1 " (~a)~%" (pretty-type t))
                         (if (eq? t (flag->type f))
                             (lp flags ns)
                             #f))
                       (begin
