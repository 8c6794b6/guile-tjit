    ...
    (define (gen-fpr)
      (if (< fpr-idx *num-fpr*)
          (let ((ret (make-fpr fpr-idx)))
            ...
            ret)
          (gen-mem)))
    (define (set-arg-types! op dst args)
                  ...
                  ((hashq-ref env arg)
                   => (lambda (reg)
                        (debug 1 ";;; set-types!: ~a already assigned to ~a~%"
                               arg reg)))
                  ((= type int)
                   (debug 1 ";;; set-arg-types!: ~a to gpr ~a~%" arg gpr-idx)
                   (hashq-set! env arg (gen-gpr)))
                  ((= type double)
                   (debug 1 ";;; set-arg-types!: ~a to fpr ~a~%" arg fpr-idx)
                   (hashq-set! env arg (gen-fpr)))
                  (else
                   (debug 1 ";;; set-types!: unknown type ~a~%" type)))
                 (debug 1 ";;; set-arg-types!: skipping constant ~a~%" arg)))
            (_
             (values))))))
