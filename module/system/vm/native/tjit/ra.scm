...
(define (compile-primlist term env gpr-idx fpr-idx mem-idx
                          snapshot-idx snapshots assign-args?)
  ...
  (let ((arg-vars '()))
    ...
    (define (constant v)
      `(const . ,v))
    (define (compile-term term snapshot-id acc)
      (match term
        (('lambda args rest)
         (debug 1 ";;; compile-term:~%")
         (debug 1 ";;;   args: ~a~%" args)
         ...
         (compile-term rest snapshot-id acc))
        (('begin term1 term2)
         (let-values (((acc snapshot-id)
                       (compile-term term1 snapshot-id acc)))
           (compile-term term2 snapshot-id acc)))
        (('let ((dst (? constant? val))) term1)
         (let ((prim `(%move ,(ref dst) ,(constant val))))
           (compile-term term1 snapshot-id (cons prim acc))))
        (('let ((dst (? symbol? src))) term1)
         (let ((prim `(%move ,(ref dst) ,(ref src))))
           (compile-term term1 snapshot-id (cons prim acc))))
        (('let (('_ ('%snap id . args))) term1)
         (let ((prim `(%snap ,id ,@(map ref args))))
           (compile-term term1 id (cons prim acc))))
        (('let (('_ (op . args))) term1)
        ...
        (('let ((dst (op . args))) term1)
         ...
         (let ((arg-regs (map ref args)))
           ...
           (let* ((dst-reg (ref dst))
             ...
             (compile-term term1 snapshot-id (cons prim acc)))))
        (('loop . _)
         (values acc snapshot-id))
        (('%snap id . args)
         (values (cons `(%snap ,id ,@(map ref args)) acc) id))
        (((? symbol? op) . args)
         (set-types! op #f args)
         (values (cons `(,op ,@(map ref args)) acc) snapshot-id))
        (()
         (values acc snapshot-id))))
    (let-values (((plist snapshot-id)
                  (compile-term term snapshot-idx '())))
