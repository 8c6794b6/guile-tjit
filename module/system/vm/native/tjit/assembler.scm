    ...
    ((_ name op-ii op-ri op-rr)
     (define-native (name (int a) (int b))
       (case (ref-type a)
         ...
         ((gpr)
          (case (ref-type b)
            ((con) (jump (op-ri (gpr a) (constant b)) (bailout)))
         ...
         ((fpr)
          (fpr->gpr r0 (fpr a))
          (case (ref-type b)
            ((con) (jump (op-ri r0 (constant b)) (bailout)))
            ((gpr) (jump (op-rr r0 (gpr b)) (bailout)))
            ((fpr) (jump (op-rr r0 (fpr->gpr r1 (fpr b))) (bailout)))
            ((mem) (jump (op-rr r0 (memory-ref r1 b)) (bailout)))
            (else (err))))
     ...
     (define-native (name (double a) (double b))
       (case (ref-type a)
         ...
         ((gpr)
          (gpr->fpr f0 (gpr a))
          (case (ref-type b)
            ((con) (jump (op-ri f0 (constant b)) (bailout)))
            ((gpr) (jump (op-rr f0 (gpr->fpr f1 (gpr b))) (bailout)))
            ((fpr) (jump (op-rr f0 (fpr b)) (bailout)))
            ((mem) (jump (op-rr f0 (memory-ref/f f1 b)) (bailout)))
            (else (err))))
         ((fpr)
          (case (ref-type b)
            ((con) (jump (op-ri (fpr a) (constant b)) (bailout)))
...
(define-native (%eqv (int a) (int b))
  (let ((volatiles (asm-volatiles asm))
    ...
    (for-each store-volatile volatiles)
    (jit-prepare)
    (let ((b/r (if (equal? a (argr 1))
                   (begin (jit-movr r1 b/r)
                          r1)
                   b/r)))
      (jit-pushargr a/r)
      (jit-pushargr b/r))
    ...
    (letrec-syntax
        ((store-b (syntax-rules ()
         ...
         (adjust (syntax-rules ()
                   ((_ dst/r b/r)
                    (begin
                      ;; Load b, check the sign.
                      (load-b b/r)
                      (jump (jit-blti b/r (imm 0)) neg-b)

                      ;; b is positive, jump to `proceed' if 0 <= dst.
                      (jump (jit-bgei dst/r (imm 0)) proceed)

                      ;; b is positive, dst is negative, adjusting dst.
                      (jit-addr dst/r dst/r b/r)
                      (jump (jit-jmpi) proceed)

                      ;; b is negative. Jump to `proceed' if dst <= 0.
                      (jit-link neg-b)
                      (jump (jit-blei dst/r (imm 0)) proceed)

                      ;; b is negative, dst is positive, adjusting dst.
                      (jit-addr dst/r dst/r b/r))))))
      (case (ref-type dst)
        ((gpr)
         (let ((dst/r (gpr dst))
      ...
      ((op3
        (syntax-rules ()
          ((_ dst)
           (begin
             (case (ref-type src)
               ((con) (begin (jit-movi-d f0 (constant src))
